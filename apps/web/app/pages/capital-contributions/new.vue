...
async function onSubmit() {
  ...
  try {
    await request('/api/v1/capital-contributions', {
      method: 'POST',
    ...
    error.value =
      'Failed to record the contribution — check the amount format (e.g. 5000.00), that the Equity account is type Equity, and the Cash account is type Asset.'
  } finally {
...
<template>
  <div class="max-w-md space-y-4">
    <h1 class="text-xl font-semibold">Record a capital contribution</h1>
    <form class="space-y-3 rounded border border-gray-200 bg-white p-4" @submit.prevent="onSubmit">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700">Cash account (money in)</label>
        <select
