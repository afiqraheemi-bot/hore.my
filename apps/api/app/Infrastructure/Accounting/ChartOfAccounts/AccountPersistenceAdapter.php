 ...
 * none.
 *
 * **Account Type persistence.** AETS-005 does not lock a database
 * representation for Account Type (§10, §25). Rather than convert the
 * domain enum into a backed enum merely to support persistence, this
 * adapter privately translates each case to and from its own PHP enum
 * case name (`AccountType::Asset->name === 'Asset'`, and so on) — a
 * deterministic, self-describing representation already inherent to
 * the type, not an invented numbering or abbreviation scheme. This
 * translation is confined entirely to this adapter; `AccountType`
 * itself remains unbacked and unaware of it.
 *
...
final class AccountPersistenceAdapter
{
     ...
     * {@see InvalidPersistedAccountTypeException}. Normal Balance is
