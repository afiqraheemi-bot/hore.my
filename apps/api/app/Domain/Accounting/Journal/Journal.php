 ...
 *
 * **Financial date and posted-at (M8, AETS-004 §9.1).** Every Journal
 * carries a caller-supplied `financialDate` — the ledger-authoritative
 * accounting date, distinct from when the Journal was actually
 * recorded — and a `postedAt` moment that is `null` for a Draft and
 * always present, and immutable, once Posted. Neither is ever
 * self-generated by this class (no `new \DateTimeImmutable('now')`
 * anywhere here) — both are supplied by the caller, exactly like
 * `JournalId` itself, keeping this class's own construction and
...
final class Journal
{
     ...
     * state differs.
     *
     * **Financial date (M8, AETS-004 §9.1).** `$financialDate` is the
     * ledger-authoritative accounting date this Journal's effect
     * belongs to — always caller-supplied, never defaulted to "now" or
     * derived from anything else, exactly like `$id` itself. It is
     * distinct from, and never conflated with, `postedAt` (§9.1): a
     * Journal may be posted today for an effect dated last month.
     *
     * @param  list<JournalLine>  $lines
     *
     ...
     */
    public static function create(TenantId $tenantId, JournalId $id, array $lines, \DateTimeImmutable $financialDate): self
    {
        return self::assembleValidated($tenantId, $id, $lines, JournalState::Draft, null, null, $financialDate, null);
    }
     ...
     * @throws InconsistentPostedAtException if `$state` and `$postedAt`
     *                                       disagree (M8) — a Draft with a `postedAt`, or a Posted
     *                                       Journal with none.
     */
    public static function reconstitute(
        ...
        return self::assembleValidated($tenantId, $id, $lines, $state, $correctionType, $correctedJournalId, $financialDate, $postedAt);
    }
     ...
     *
     * **`$postedAt` is caller-supplied, not self-generated (M8).**
     * Mirroring `JournalId`'s own established convention (AETS-004
     * §11) — this method never calls `new \DateTimeImmutable('now')`
     * itself, which would make it impure and non-deterministic. The
     * caller (the orchestration layer already responsible for the
     * atomic persistence this method's result feeds into) supplies the
     * exact moment durable posting occurs.
     *
     ...
     *
     * **`$financialDate` is caller-supplied, never derived (M8).** This
     * method does not copy the original Journal's own `financialDate`,
     * default to "today", or otherwise decide a policy for it —
     * whether a Reversal's financial date should match the original,
     * the correction date, or something else entirely is a
     * policy/calling-layer concern this method has no authority to
     * settle, deferred to Posting Rules/Period Management (AETS-006).
     *
     ...
     */
    public function reverse(JournalId $newJournalId, \DateTimeImmutable $financialDate): self
    {
        if ($this->state !== JournalState::Posted) {
     ...
     *                                    Journal Lines.
     *
     * **`$financialDate` is caller-supplied, never derived (M8)** — the
     * identical reasoning as {@see reverse()}'s own docblock.
     *
     ...
     *                                    not exactly equal total Credit Money.
     */
    public static function createReplacement(TenantId $tenantId, JournalId $newJournalId, array $lines, self $reversal, \DateTimeImmutable $financialDate): self
    {
    ...
    /**
     * The ledger-authoritative accounting date this Journal's effect
     * belongs to (M8, AETS-004 §9.1) — always present, distinct from
     * {@see postedAt()}. This is the single date future Trial
     * Balance/P&L/period-based reporting relies on to determine which
     * Accounting Period a Journal belongs to, without ever joining
     * back to whatever business module produced it.
     */
    ...
    /**
     * The moment this Journal was durably Posted (M8, AETS-004 §9.1) —
     * `null` for a Draft Journal, always present and immutable for a
     * Posted one. Distinct from {@see financialDate()}: a Journal
     * posted today can carry a financial date from last month.
     */
     ...
     * @throws InconsistentPostedAtException
     */
    private static function assembleValidated(
        ...
        if (($state === JournalState::Posted) !== ($postedAt !== null)) {
            throw InconsistentPostedAtException::forJournalId($id);
        }

        $journal = new self($tenantId, $id, $lines, $state, $correctionType, $correctedJournalId, $financialDate, $postedAt);
