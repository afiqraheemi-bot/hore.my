...
/**
 * An Income's stable, opaque identifier (M9) — assigned at creation
 * and immutable for the Income's lifetime, never reused or reassigned
 * to a different Income.
 *
 ...
 * obtains a fresh, collision-free IncomeId (a UUID, a ULID, or
 * otherwise) remains deferred, exactly like JournalId's own generation
 * strategy.
 *
 * **Distinct from JournalId, deliberately.** An Income (Transactions
 ...
 * Income its own identity space independent of Accounting Core's.
 */
final class IncomeId
{
    ...
    /**
     * @throws InvalidIncomeIdException if the value is not a canonical
     *                                  Income identifier.
     */
    public static function of(string $value): self
    {
        if ($value === '' || strlen($value) > self::MAX_LENGTH) {
            throw InvalidIncomeIdException::forValue($value);
        }
