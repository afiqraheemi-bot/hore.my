 ...
 * proof of {@see TaskService::approve()}'s TSK-004 guard (WTS-001 §6):
 * two concurrent `Approved` attempts against the same Task must never
 * both succeed. Mirrors {@see \Tests\bin\concurrent_deallocate_worker}'s
 * own reasoning and synchronization technique exactly.
 *
 * Usage: `php concurrent_task_approve_worker.php <tenantId> <taskId>
...
try {
    $task = $service->approve($tenantId, $taskId, $actor);
