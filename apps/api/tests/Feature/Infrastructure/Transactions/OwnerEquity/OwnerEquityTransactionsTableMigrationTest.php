...
namespace Tests\Feature\Infrastructure\Transactions\OwnerEquity;
...
/**
 * Integration-level proof for the production `owner_equity_transactions`
 * table migration (M15,
 ...
 * role), the `movement_type` canonical-value `CHECK` constraint, and
 * the primary key {@see OwnerEquityTransactionRepository} relies on.
 *
 ...
 * an explicit reason.
 */
final class OwnerEquityTransactionsTableMigrationTest extends TestCase
{
    ...
    private const MIGRATION_PATH = 'database/migrations/2026_09_07_060000_create_owner_equity_transactions_table.php';
    ...
    protected function setUp(): void
    {
        ...
        $this->seedAccount('tenant-0001', 'account-owner-capital', 'Equity');
        $this->seedAccount('tenant-0001', 'account-bank', 'Asset');
        $this->seedAccount('tenant-0002', 'account-owner-capital-b', 'Equity');
    ...
    public function test_table_creates_successfully(): void
    {
        ...
        $this->assertTrue(Schema::connection('pgsql')->hasColumns(self::TABLE, [
            'id', 'tenant_id', 'journal_id', 'movement_type', 'amount', 'currency', 'transaction_date',
            'equity_account_id', 'cash_account_id', 'description', 'evidence_reference', 'recorded_at',
        ]));
    }

    public function test_valid_contribution_inserts(): void
    {
        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0001', 'Contribution', 'account-owner-capital', 'account-bank');

        $this->assertSame(1, DB::connection('pgsql')->table(self::TABLE)->count());
    }

    public function test_valid_drawing_inserts(): void
    {
        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0001', 'Drawing', 'account-owner-capital', 'account-bank');

        $this->assertSame(1, DB::connection('pgsql')->table(self::TABLE)->count());
    }

    public function test_a_non_canonical_movement_type_is_rejected(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0001', 'Withdrawal', 'account-owner-capital', 'account-bank');
    }
    ...
    public function test_transaction_cannot_reference_a_nonexistent_journal(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-does-not-exist', 'Contribution', 'account-owner-capital', 'account-bank');
    }

    public function test_transaction_cannot_reference_another_tenants_journal(): void
    {
        $this->expectException(QueryException::class);

        // journal-0002 exists, but only under tenant-0002.
        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0002', 'Contribution', 'account-owner-capital', 'account-bank');
    }

    public function test_transaction_cannot_reference_a_nonexistent_equity_account(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0001', 'Contribution', 'account-does-not-exist', 'account-bank');
    }

    public function test_transaction_cannot_reference_another_tenants_cash_account(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransaction('owner-equity-0001', 'tenant-0001', 'journal-0001', 'Contribution', 'account-owner-capital', 'account-owner-capital-b');
    }
    ...
    public function test_recorded_at_is_present_and_database_assigned(): void
    {
        ...
        $row = DB::connection('pgsql')->table(self::TABLE)->where('id', 'owner-equity-0001')->first();
    ...
    private function insertTransaction(string $id, string $tenantId, string $journalId, string $movementType, string $equityAccountId, string $cashAccountId): void
    {
        DB::connection('pgsql')->table(self::TABLE)->insert([
            ...
            'description' => 'Test owner equity transaction',
