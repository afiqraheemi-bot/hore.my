...
namespace Tests\Feature\Infrastructure\Transactions\Transfer;
...
/**
 * Integration-level proof for the production `transfers` table migration
 * (M14, `database/migrations/2026_09_07_050000_create_transfers_table.php`),
 ...
 * role) and the primary key {@see TransferRepository} relies on.
 *
 ...
 * an explicit reason.
 */
final class TransfersTableMigrationTest extends TestCase
{
    ...
    private const MIGRATION_PATH = 'database/migrations/2026_09_07_050000_create_transfers_table.php';
    ...
    protected function setUp(): void
    {
        ...
        $this->seedAccount('tenant-0001', 'account-petty-cash', 'Asset');
    ...
    public function test_table_creates_successfully(): void
    {
        ...
        $this->assertTrue(Schema::connection('pgsql')->hasColumns(self::TABLE, [
            ...
            'source_account_id', 'destination_account_id', 'description', 'evidence_reference', 'recorded_at',
        ]));
    }

    public function test_valid_transfer_inserts(): void
    {
        $this->insertTransfer('transfer-0001', 'tenant-0001', 'journal-0001', 'account-bank', 'account-petty-cash');
    ...
    public function test_transfer_cannot_reference_a_nonexistent_journal(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransfer('transfer-0001', 'tenant-0001', 'journal-does-not-exist', 'account-bank', 'account-petty-cash');
    }

    public function test_transfer_cannot_reference_another_tenants_journal(): void
    {
        $this->expectException(QueryException::class);

        // journal-0002 exists, but only under tenant-0002.
        $this->insertTransfer('transfer-0001', 'tenant-0001', 'journal-0002', 'account-bank', 'account-petty-cash');
    }

    public function test_transfer_cannot_reference_a_nonexistent_source_account(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransfer('transfer-0001', 'tenant-0001', 'journal-0001', 'account-does-not-exist', 'account-petty-cash');
    }

    public function test_transfer_cannot_reference_another_tenants_destination_account(): void
    {
        $this->expectException(QueryException::class);

        $this->insertTransfer('transfer-0001', 'tenant-0001', 'journal-0001', 'account-bank', 'account-bank-b');
    }
    ...
    public function test_recorded_at_is_present_and_database_assigned(): void
    {
        ...
        $row = DB::connection('pgsql')->table(self::TABLE)->where('id', 'transfer-0001')->first();
    ...
    private function insertTransfer(string $id, string $tenantId, string $journalId, string $sourceAccountId, string $destinationAccountId): void
    {
