...
/**
 * Covers `IncomeToPostingCommandTranslator` (M9) — the sole bridge
 * between a `RecordIncomeCommand` (Transactions domain) and a
 * `PostingCommand` (Accounting Core). Proves the fixed accounting
 * mapping (Debit Deposit Account, Credit Income Account), that no
 * Source Fingerprint is ever supplied, and that the Source reference is
 * self-referential and traceable, never fabricated.
 */
final class IncomeToPostingCommandTranslatorTest extends TestCase
{
    private IncomeToPostingCommandTranslator $translator;
    ...
    protected function setUp(): void
    {
        ...
        $this->translator = new IncomeToPostingCommandTranslator;
        $this->tenantId = TenantId::of('tenant-0001');
        $this->amount = Money::fromDecimalString('75.50', Currency::of('MYR'));
    }

    public function test_produces_a_debit_deposit_credit_income_pair(): void
    {
     ...
     * Date is exactly the Income command's own `transactionDate()` —
     * never `created_at`, never "today", never any other value.
     */
    public function test_financial_date_is_the_incomes_own_transaction_date(): void
    {
    ...
    public function test_source_reference_is_self_referential_to_the_income(): void
    {
        $command = $this->makeCommand();
        $postingCommand = $this->translator->translate($command);

        $this->assertSame('income:'.$command->incomeId()->toString(), $postingCommand->source()->toString());
    }
    ...
    private function makeCommand(?EvidenceReference $evidenceReference = null): RecordIncomeCommand
    {
