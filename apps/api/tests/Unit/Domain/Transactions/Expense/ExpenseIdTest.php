...
/**
 * Covers Expense identity (M7) as a bare Value Object, mirroring
 * {@see JournalIdTest}'s own
 * coverage of the pattern this class deliberately reuses.
 */
final class ExpenseIdTest extends TestCase
{
    public function test_valid_opaque_identifier_is_accepted(): void
    {
        $id = ExpenseId::of('01J8Z3K7QYUXG5N7EXAMPLE01');

        $this->assertInstanceOf(ExpenseId::class, $id);
    }

    public function test_exact_string_round_trip(): void
    {
        $id = ExpenseId::of('expense-0001');

        $this->assertSame('expense-0001', $id->toString());
    }

    public function test_same_value_is_equal(): void
    {
        $a = ExpenseId::of('expense-0001');
        $b = ExpenseId::of('expense-0001');
    ...
    {
        $this->expectException(InvalidExpenseIdException::class);
    ...
    public function test_value_one_character_past_the_length_bound_is_rejected(): void
    {
        ...
        ExpenseId::of(str_repeat('a', 65));
    }

    public function test_expense_id_is_immutable(): void
    {
        $reflection = new ReflectionClass(ExpenseId::class);
    ...
    {
        $expenseId = ExpenseId::of('shared-value-0001');
        $journalId = JournalId::of('shared-value-0001');

        $this->assertNotInstanceOf(JournalId::class, $expenseId);
        $this->assertNotInstanceOf(ExpenseId::class, $journalId);
    }
}
