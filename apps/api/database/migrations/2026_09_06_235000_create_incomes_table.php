...
/**
 * Production schema for the Income record (M9) — the Transactions
 * domain's authoritative business context for a manually-recorded
 * income, kept traceable to the Posted Journal it produced.
 *
 ...
 * posting (amount, transaction date, Income Account, Deposit Account,
 ...
 * `tenant_id`, `journal_id`, `income_account_id`, `deposit_account_id`
 * are all `string(64)` — the same defensive opaque-identifier bound
 * already established throughout this schema. `id` (the Income's own
 * identity) is likewise `string(64)`, mirroring `journal_id`'s own
 * physical shape without being the same identity space (M9's own
 * `IncomeId`, distinct from `JournalId`).
 *
 * **Three composite foreign keys, all tenant-safe** — `(tenant_id,
 * journal_id)` onto `journals`, `(tenant_id, income_account_id)` and
 * `(tenant_id, deposit_account_id)` onto `accounts` — the identical
 ...
 * durably recorded the Income, database-assigned, purely
 * observational, mirroring `audit_events.occurred_at`).
 *
 * **`evidence_reference` is nullable** — an Income with no independent
 ...
 *
 * **No `updated_at`.** An Income is immutable once recorded — a
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create(self::TABLE, function (Blueprint $table): void {
            ...
            $table->string('income_account_id', 64);
            ...
            $table->foreign(['tenant_id', 'income_account_id'])
