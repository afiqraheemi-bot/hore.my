...
- **TSK-013 (added v3.0.0):** A submitter may explicitly defer the Account decision at submission time — the one real, human-initiated, non-AI reason `Processing`→`NeedsInformation` is reachable today, matching §3's own "ambiguous or incomplete evidence" framing. Such a Task is created and moved directly to `NeedsInformation`, atomically, with zero Proposal ever created — the submitted Command type, amount, transaction date, description, and evidence reference are retained in a Task Draft (a Workspace/Task-owned record, never a Proposal and never validated against Accounting Core) so they need not be re-entered. Completing the deferred decision (supplying both Account references) performs `NeedsInformation`→`Processing`→`NeedsReview` in one atomic step, running the retained Draft fields through the exact same Proposal validation and construction `submit()` itself performs (TSK-003) — a rejected Account reference here fails the request and leaves the Task in `NeedsInformation` unchanged, never partially advanced and never discarding the Draft.
- **TSK-014 (added v3.0.0):** Extends TSK-008's "creates a new Task referencing the original" principle to the `NeedsReview`→`Superseded` transition §4 already lists: a Task under review may be corrected, not only rejected — superseding it and submitting its replacement is one atomic operation, and the replacement Task carries an immutable, opaque `supersedesTaskId` naming the superseded Task. This is the concrete "correction-Task-linking mechanism" [WT-001](tests/WT-001-Task-Proposal-State-Model-Test-Specification.md) §7 recorded as not yet existing for TSK-008's own second clause. `supersedesTaskId` is opaque — never interpreted, never validated against the superseded Task's own content — mirroring an Evidence Reference's own opacity rule ([AETS-010](../accounting/AETS-010-Audit-Trail-Evidence-Linkage.md) §8).
...
- **3.0.0 (2026-09-16):** Closes two of the eight target-experience gaps the Founder's own review flagged as still open ("NeedsInformation dan edit/supersede Proposal UX"), by giving `NeedsInformation` and `NeedsReview`→`Superseded` their first real orchestration and human-facing purpose — both transitions were already listed in §4 but, until now, unreachable by any code path. TSK-013 defines the one real, non-AI trigger for `NeedsInformation` (an explicitly deferred Account decision, held in a new Task Draft record) and its completion path back through `NeedsReview`. TSK-014 extends TSK-008's correction principle to `NeedsReview`, via a new opaque `supersedesTaskId` link, closing the gap [WT-001](tests/WT-001-Task-Proposal-State-Model-Test-Specification.md) §7 had recorded against TSK-008's own second clause. Per this document's own MAJOR-version rule (a change altering a contract this document describes). TSK-001–TSK-012 are unchanged.
