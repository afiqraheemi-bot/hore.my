...
| [WTS-001 — Task & Proposal State Model](WTS-001-Task-Proposal-State-Model.md) | Active | 3.0.0 | Task states, allowed transitions, the Proposal contract, the transition-audit record shape (`TSK-001`), and 14 `TSK-NNN` invariants covering transition validity, Proposal/Command validation parity, approval-time idempotency (`TSK-004`), closed-Period re-validation at execution time (`TSK-005`), the AI/human approval boundary (`TSK-006`), tenant isolation (`TSK-007`), terminal-state discipline (`TSK-008`–`TSK-009`), atomic multi-row writes (`TSK-010`), Task-submission payload-conflict detection (`TSK-011`), crash recovery for a Task stranded in `Executing` (`TSK-012`), and — as of v3.0.0 — the deferred-Account-decision `NeedsInformation` path (`TSK-013`) and the edit/supersede correction path for a Task under review (`TSK-014`) |
...
| --- | --- | --- | --- | --- |
| [WT-001 — Task & Proposal State Model Test Specification](tests/WT-001-Task-Proposal-State-Model-Test-Specification.md) | Active | 1.3.0 | [WTS-001](WTS-001-Task-Proposal-State-Model.md) | 82 test IDs (`TSK-T001`–`TSK-T082`) tracing `TSK-001`–`TSK-014` across unit, isolated real-PostgreSQL migration, service integration, HTTP, concurrency, fault-injection and real-browser E2E tests, with partially deferred coverage recorded explicitly for `TSK-006` and `TSK-008` |
