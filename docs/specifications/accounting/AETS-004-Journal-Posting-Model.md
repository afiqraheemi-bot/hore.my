...
- A Journal MUST carry a Financial Date, and MUST record a posting time once Posted (§9.1) — neither is ever the same value as, or silently substituted by, `created_at` or any other system timestamp unrelated to the accounting fact it represents.
...
### 9.1 Financial date and posted-at (M8)

The Malaysian solopreneur SRS ([`hore.my_Spesifikasi_Keperluan_Sistem_v1.0_BM.pdf`](../../product/reference/hore.my_Spesifikasi_Keperluan_Sistem_v1.0_BM.pdf) §5.1) requires that financial date, posting time, and source time be stored separately from one another. This section resolves, for the Journal aggregate specifically, which of those three concepts the Journal owns.

- **Financial Date — the Journal owns it, authoritatively.** Every Journal, in every state, MUST carry a `financialDate`: the accounting date its effect belongs to (`JRN-024`, §22). It is set once, when the Journal is first assembled as a Draft (whether or not that Draft is separately persisted before posting, §9), and it MUST NOT change afterward, for either a Draft or a Posted Journal. This is a calendar date, not a moment — it identifies an accounting *day*.
- **Financial Date is never self-generated.** Neither the Journal aggregate itself, nor any Posting Engine component, MUST invent a Financial Date on a caller's behalf — not from `created_at`, not from "today," not from any other system-derived value (`JRN-024`). A Posting Command (AETS-007) MUST supply it explicitly; a Reversal or Replacement request (§16, §17) MUST supply its own explicitly, with no policy of this document deriving one from the Journal it corrects.
- **Why the Journal owns Financial Date, not only the calling Business Transaction.** A future Reporting/Period Management module (AETS-009, AETS-014) MUST be able to determine which accounting period a Journal's effect belongs to from the Journal alone — without joining every calling module's own table (Expense, Income, Invoice, Bank, and so on). Each such module MAY additionally retain its own business-facing date on its own record; that date is copied onto the Journal's `financialDate` at posting time (AETS-007 §11.1), and the two MAY subsequently be compared for traceability, but only the Journal's own `financialDate` is ledger-authoritative.
- **Posted-at — a system timestamp, distinct from Financial Date.** A Journal additionally records `postedAt`: the moment it durably transitioned from Draft to Posted (`JRN-025`, §22). It is `null` for a Draft Journal; the *first* transition to Posted sets it, once; it MUST NOT change afterward, including on an idempotent replay of an already-posted command (§14) — a replay MUST return the original `postedAt`, never a new one. A Journal assembled and posted in the same call still records the real durable-commit moment, not an earlier in-memory assembly moment.
- **Posted-at is never self-generated by the Journal aggregate either.** The orchestration layer responsible for the Draft → Posted transition supplies the current moment explicitly (mirroring how `JournalId` is always supplied, never self-generated, §11) — this keeps the Journal aggregate itself a pure, deterministic function of its inputs, with no hidden dependency on the wall clock.
- **Source time is out of scope for the Journal aggregate.** The moment the underlying source data (a receipt, a bank statement line, an external payload) itself originated is a fact about that source, not about the Journal — it belongs to the calling module's own record or to a future Evidence/source specification (AETS-010), never to a third Journal-level timestamp invented here.
- **Correction financial-date policy remains deferred.** Which Financial Date a Reversal or Replacement should carry — the original's date, the correction's own date, or some other policy — is a Posting Rules/Period Management decision (§2.2, §26; AETS-006), not resolved by this section. `Journal::reverse()`/`Journal::createReplacement()` (§16, §17) accept it as an explicit, caller-supplied argument and derive it from nothing.
...
- A Reversal MUST carry its own explicit Financial Date (`JRN-024`, §9.1), supplied by its caller — never derived from the original Journal's Financial Date, and never defaulted to "today." Which date is appropriate is a Posting Rules policy decision this document does not resolve (below).
...
- A Replacement MUST carry its own explicit Financial Date (`JRN-024`, §9.1), supplied by its caller — never derived from its Reversal's or the original Journal's Financial Date, and never defaulted to "today," for the same reason stated for Reversal (§16).
...
| JRN-024 | **Financial Date always present, never self-generated.** Every Journal, in every state, MUST carry a Financial Date, supplied explicitly by the caller at assembly time; it MUST NOT be derived from `created_at` or any other system timestamp, and MUST NOT change once set (§9.1). |
| JRN-025 | **Posted-at set exactly once, at the durable posting moment.** A Draft Journal's `postedAt` MUST be absent; the Draft → Posted transition MUST set it exactly once, to the durable commit moment; it MUST NOT change afterward, including on an idempotent replay (§9.1). |
...
- **Financial Date and posted-at tests** — proving a Journal's Financial Date is persisted exactly as supplied and never changes after posting; a Draft Journal's `postedAt` is absent; the Draft → Posted transition sets `postedAt` exactly once; an idempotent replay preserves the original `postedAt` unchanged; and a Reversal/Replacement each persist their own explicitly supplied Financial Date, never one derived from the Journal they correct (`JRN-024`, `JRN-025`, §9.1).
...
- **Detailed correction workflow mechanics** — whether/how many times a Journal may be reversed, approval requirements for a Replacement, period-close interaction with posting, and which Financial Date a Reversal or Replacement should be constructed with (§9.1, §16, §17; AETS-006, Posting Rules, per [AETS-000 §10](AETS-000.md#10-planned-document-structure)).
- **"Latest replacement" / "current corrected Business Transaction" semantics** — whether, and how, a calling module (Expense, and any future Income/Invoice/Bank consumer) should present a single "current" corrected state after a correction chain is not resolved by this document; correction cardinality and any such workflow concept remains deferred to AETS-006. This document's own correction-chain fields (`correctionType`, `correctedJournalId`, §6) are sufficient for a caller to walk the chain itself; it does not assume the chain has only one child.
...
- **2.0.0 (2026-09-06):** Resolved a cross-cutting gap discovered during the M7 Post-Closure Architecture Check: [SRS §5.1](../../product/reference/hore.my_Spesifikasi_Keperluan_Sistem_v1.0_BM.pdf) requires financial date, posting time, and source time stored separately, but this document never gave the Journal aggregate a Financial Date or a posting-time field at all — leaving a calling module (M7's Expense, and any future Income/Invoice/Bank consumer) with no way to make the ledger's own accounting date authoritative, and a future Reporting/Period Management module unable to determine period membership from the Journal alone. Adds **§9.1 (Financial date and posted-at)**: every Journal now carries a caller-supplied, never-self-generated `financialDate` (`JRN-024`) set once at Draft assembly and immutable thereafter; and a `postedAt` (`JRN-025`) that is `null` for a Draft, set exactly once at the durable Draft → Posted transition (supplied by the orchestration layer, never generated by the Journal aggregate itself, mirroring the existing `JournalId` no-self-generation convention, §11), and immutable afterward, including across an idempotent replay. §6 gains a summary bullet; §16/§17 each gain a rule that a Reversal/Replacement carries its own explicitly-supplied Financial Date, never one derived from the Journal it corrects — that selection remains a Posting Rules/Period Management policy decision, deferred to AETS-006 (§26, expanded). §26 also adds an explicit deferral: this document does not define "latest replacement" or "current corrected Business Transaction" semantics — a correction chain's cardinality and any such workflow concept remains AETS-006's to design; a caller walks the existing `correctionType`/`correctedJournalId` chain itself. §22 gains `JRN-024`/`JRN-025` (appended — no existing `JRN-NNN` ID renumbered, altered, or removed), and §25 gains a corresponding ATS requirement. The companion [AETS-007](AETS-007-Posting-Command.md) v2.0.0 amendment (same task) adds Financial Date as a new required Posting Command field carrying this value in. Classified **MAJOR** per §27: `Journal::create()`/`reconstitute()`/`post()`/`reverse()`/`createReplacement()` each gain a new required parameter with no default — a breaking change to this document's own already-Active aggregate contract, not an additive clarification.
