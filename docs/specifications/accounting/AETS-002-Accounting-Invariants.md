...
3. **Idempotency and source deduplication.** Every material command that can create an authoritative accounting effect carries an idempotency key; repeating or concurrently retrying the same logical command returns the original economic result and never creates a second posting. A command that originates from, or is materially derived from, an external or imported source for which duplicate-source detection is required (for example, a bank source line, an uploaded receipt or invoice, an imported statement row, or an external integration payload) additionally carries a source fingerprint, and MUST fail safely, before any persistent effect, if that required source fingerprint is missing. A source fingerprint MUST NOT be fabricated merely to satisfy this requirement where no such source exists.
...
- **2.0.0 (2026-09-05):** Corrected invariant 3 (previously "Idempotency," now "Idempotency and source deduplication") — a genuine internal contradiction discovered while drafting AETS-007 (Posting Command specification, M4-T0): this invariant's literal wording required an idempotency key *and* a source fingerprint on every material command, but [AETS-001](AETS-001-Accounting-Terminology.md#idempotency-key)'s own Idempotency Key entry cites this invariant and states "required on every material command," while its Source Fingerprint entry cites invariant 8 instead and never claims universality — the two glossary entries already treated the fields asymmetrically. [AETS-004](AETS-004-Journal-Posting-Model.md#14-idempotency) (Active, governs current Posting Command idempotency behavior) independently never required a source fingerprint at all. The corrected wording makes explicit what both sibling documents already assumed: idempotency key remains universal and unconditional (the operative "never creates a second posting" guarantee, unchanged); source fingerprint applies only where a command originates from, or is materially derived from, external/imported source data requiring duplicate-source detection — and such a command MUST still fail safely, before any persistent effect, if that required fingerprint is missing (duplicate prevention is not weakened; a new explicit fail-safe requirement and an explicit anti-fabrication rule are added). This is classified as a **MAJOR** change under [AETS-000 §9.1](AETS-000.md#91-per-document-version)'s own worked example ("a changed idempotency scope"), even though no previously-Active document ever enforced the literal universal reading in practice. The review [AETS-000 §8.2](AETS-000.md#82-ownership-and-review) requires for a MAJOR change has been completed: the CTO/Technical Partner and Accounting Domain Reviewer approved this correction — the universal Idempotency Key requirement, the conditional Source Fingerprint requirement, and this MAJOR classification itself (M4-T0.2, Founder-ratified). No other invariant, and no other part of invariant 3's core guarantee, changed.
