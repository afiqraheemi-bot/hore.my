...
**Column boundaries tolerate a missing space (added 2026-09-20, real-usage bug fix).** After the encryption fix above let a second real statement through to the parser itself, one of its lines failed with "Line 1 could not be recognized as a transaction..." — `smalot/pdfparser`'s own text extraction had rendered it as `01/01/26IBK FUND TFR FR A/C 66.60-257.05`, with no space at all between the date and the description, nor between the trailing sign and the running balance, despite every synthetic `dompdf`-built test fixture always producing one at those exact boundaries. The transaction-line and `BEGINNING BALANCE` patterns now use optional (`\s*`) rather than required (`\s+`) whitespace at every boundary — the date's fixed `DD/MM/YY` width and the amount/balance's own `\d+\.\d{2}` decimal shape already delimit these boundaries unambiguously without relying on whitespace being present, so this costs nothing for a statement that *does* space them out while fixing the one that does not.

**Handles the common encrypted-PDF case (added 2026-09-20, real-usage bug fix).** A real user-uploaded Maybank statement failed with "Secured pdf file are currently not supported" — `smalot/pdfparser` has no PDF-decryption support of its own at all, and the bank's own PDF is permissions-only encrypted with an *empty* user password (opens with no prompt in any real PDF viewer — the overwhelmingly common real-world case for bank-issued statement PDFs). `QpdfDecryptor` (`app/Infrastructure/Banking`) strips this transparently, via the `qpdf` system binary (`--decrypt`, no password argument needed for this case), before every PDF import — the one system-binary dependency this codebase's PDF handling has; every other PDF concern (Dompdf output, `smalot/pdfparser` text extraction) stays pure-PHP. A PDF locked with a genuine, non-empty password remains honestly unrecoverable (`qpdf` cannot guess a password it was never given) and now fails with actionable guidance ("please remove the password before uploading") instead of the original opaque error. Verified directly: a synthetic Maybank-shaped fixture encrypted both RC4-128 and AES-256 with an empty user password parses identically to its unencrypted original.
...
- **0.9.2 (2026-09-20):** Second real-usage bug fix to §5.2's `MaybankPdfBankStatementParser`, found immediately after v0.9.1 let a real statement reach the parser for the first time: a transaction line failed with "Line 1 could not be recognized..." because `smalot/pdfparser`'s own text extraction rendered `01/01/26IBK FUND TFR FR A/C 66.60-257.05` — no space between the date and description, nor between the sign and balance — where every synthetic test fixture had always had one. The transaction-line and `BEGINNING BALANCE` regex patterns now use optional (`\s*`) rather than required (`\s+`) whitespace at every boundary; the date's fixed width and the amount/balance's own decimal shape already delimit them unambiguously either way. Description is now explicitly trimmed when captured, guarding against either boundary's optional whitespace landing inside the captured group instead of outside it. Verified: a new regression test reproduces the exact reported line (built as one contiguous text run, not separate table cells, specifically to force the missing-space condition every other fixture's cell-boundary spacing avoids) and parses it correctly; the full existing suite (properly-spaced fixtures) stays green unchanged. No schema, invariant, or prior decision changed — a pure defect fix. Classified editorial/PATCH-equivalent.
- **0.9.1 (2026-09-20):** Real-usage bug fix to §5.2's `MaybankPdfBankStatementParser` — a real user-uploaded Maybank statement failed entirely with "Secured pdf file are currently not supported": `smalot/pdfparser` has no PDF-decryption support of its own, and the bank's own PDF is permissions-only encrypted with an empty user password (the overwhelmingly common case for real bank statement PDFs — opens with no prompt in any viewer). New `QpdfDecryptor` (`app/Infrastructure/Banking`) strips this transparently via the `qpdf` system binary before every PDF import — added to the API Dockerfile, the one system-binary dependency this codebase's PDF handling has. A genuinely password-locked PDF (a real, non-empty password) still fails, now with actionable guidance instead of the prior opaque error. Verified: a synthetic Maybank-shaped fixture encrypted both RC4-128 and AES-256 with an empty user password parses identically to its unencrypted original; a fixture locked with a real password is rejected with the new clear message. No schema, invariant, or prior decision changed — a pure defect fix to an already-implemented capability. Classified editorial/PATCH-equivalent.
