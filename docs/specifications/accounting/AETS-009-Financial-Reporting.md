...
- **Aging Report historical reproducibility across time — data retention resolved 2026-09-11 (P1-3), report logic still open.** A Payment Allocation is no longer hard-deleted ({@see AllocationService::deallocate()} now soft-deletes, retaining full history), so the audit-trail gap an external audit named is closed. The Aging Report query itself, however, still unconditionally excludes every soft-deleted allocation regardless of the requested as-of date, so a historical as-of-date Aging Report can still change if re-run after a contributing allocation is later deallocated — the data to reconstruct the correct historical figure now exists, but §17's query does not yet use it. Closing this remaining gap requires the query to compare `deleted_at` against the as-of date, not merely check `IS NULL` — deferred here, not designed in this version.
...
**A named limitation, partially resolved 2026-09-11 (P1-3): the underlying data is no longer destroyed, but this report's own calculation does not yet use that retained history.** Rule 3 (rebuildable) holds exactly as stated — two independent computations of this report against the *same* database state, for the same Tenant and as-of date, always produce identical output. `PaymentAllocation` deletion ({@see App\Domain\Payments\AllocationService::deallocate()}) is, as of 2026-09-11, a soft delete (`deleted_at`/`deleted_by_actor`) rather than a physical `DELETE` — a deallocated allocation's full history (amount, Payment, Invoice, who deallocated it, when) is now permanently retained, closing the audit-trail gap an external audit named (P1-3). This report's own query, however, still filters every allocation by `deleted_at IS NULL` unconditionally, exactly as it filtered by physical row existence before — so a *past* as-of date's result can still change *across time* if a contributing allocation is later deallocated, because the query does not yet compare `deleted_at` against the requested as-of date to reconstruct what was true *then*. Closing that remaining gap requires the query itself to reason about `deleted_at` relative to the as-of date (an allocation counts if it existed and had not yet been deallocated as of that date) — a report-logic change, not a data-retention one, deferred here as a distinct, still-open item (§15).
