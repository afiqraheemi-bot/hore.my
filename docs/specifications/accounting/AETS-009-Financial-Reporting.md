...
| RPT-014 | **Aging historical reproducibility across time.** A Debtors/Aging report computed for a given as-of date MUST return an identical result no matter when it is computed, including after a contributing Payment Allocation is later deallocated — a deallocation MUST affect only as-of dates on or after the date it took effect, never a strictly earlier one (§17). |
...
- **As of v1.3.0: Aging historical reproducibility proof** — a fully-allocated Invoice's report result for a historical as-of date proven identical before and after the contributing allocation is later deallocated, and proven to differ only for an as-of date on or after the deallocation itself (`RPT-014`, §17).
...
- ~~**Aging Report historical reproducibility across time**~~ — resolved 2026-09-11 (P1-3, then P1-4); see §17. A Payment Allocation is no longer hard-deleted, and `AgingReportQuery` now compares `deleted_at` against the requested as-of date rather than merely checking `IS NULL`, so a historical as-of-date's result is identical before and after a later deallocation.
...
**A formerly-named limitation, fully resolved 2026-09-11 (P1-3, then P1-4).** Rule 3 (rebuildable) holds exactly as stated — two independent computations of this report against the *same* database state, for the same Tenant and as-of date, always produce identical output, **and now this holds across time too**: a *past* as-of date's result is identical before and after a contributing allocation is later deallocated. `PaymentAllocation` deletion ({@see App\Domain\Payments\AllocationService::deallocate()}) is a soft delete (`deleted_at`/`deleted_by_actor`) rather than a physical `DELETE` — a deallocated allocation's full history is permanently retained (P1-3), and `AgingReportQuery` now compares `deleted_at` against the requested as-of date rather than merely checking `IS NULL`: an allocation counts toward a given as-of date if it had not yet been deallocated as of that date — `deleted_at IS NULL`, or `deleted_at`'s own calendar date is *after* the as-of date (deallocated only later than the date being reported on). `deleted_at` is a system timestamp, not a user-supplied Financial Date, but this use of it does not conflict with rule 5: it answers "had this fact taken effect yet, as of this date," never substitutes for a Financial Date on a financial event — deallocation, unlike a Payment or an Invoice, has no business-dated field of its own. Verified directly: an Invoice paid in full, queried as of a historical date, then deallocated, then re-queried for that *same* historical date, returns an identical result both times; only a report for the *current* date reflects the deallocation.
