...
- **Evidence's own retention and storage mechanics** — the physical file, its storage backend, encryption, hash verification algorithm, and retention/deletion lifecycle. [AETS-001](AETS-001-Accounting-Terminology.md#evidence) states these are "defined elsewhere." As of 2026-09-16, [AETS-015](AETS-015-Evidence-Storage.md) is that elsewhere for a first, narrow slice (a real upload/retrieval boundary and persisted Evidence record); encryption-at-rest, malware scanning, and retention/deletion policy remain deferred by AETS-015 §2.2/§10 in turn.
- **Evidence's own tenant-scoped aggregate and persisted identity.** [AETS-015](AETS-015-Evidence-Storage.md) now produces a real, persisted, tenant-scoped Evidence record (as of 2026-09-16) — this document's own reference contract (§8) and linkage record (§9) are unchanged by that; a Posting Command still carries the same opaque Evidence Reference it always did, now resolvable against something real.
...
**Evidence tenant-ownership — a tracked, honest gap, not a violation.** [AETS-007 §14](AETS-007-Posting-Command.md#14-validation-pipeline) step 1 requires "any Evidence's Tenant" to be validated against the command's own Tenant, on the same footing as an Account reference. This document still does not resolve that check itself: [AETS-015](AETS-015-Evidence-Storage.md) (2026-09-16) now persists a real, tenant-scoped Evidence record a validator *could* check against, but AETS-015 §2.2 explicitly excludes wiring that check into `PostingCommandAccountValidator` or any Command translator — that remains a separate, future change to this document or AETS-007's own validator, not resolved by AETS-015's existence alone. No Posting Command MAY treat a supplied Evidence Reference as validated in the interim beyond the format-level checks §8 already requires.
...
- **1.1.0 (2026-09-16):** Updates §2.2 and §11 to point to [AETS-015](AETS-015-Evidence-Storage.md) (Draft), which now resolves this document's own named "Evidence's own retention and storage mechanics"/"tenant-scoped aggregate" deferral for a first, narrow slice. This document's own Evidence Reference contract (§8), Evidence Linkage record (§9), and `AUD-NNN` invariants are unchanged — a Posting Command still carries the same opaque reference it always did. The Posting Command validation-pipeline wiring §11 also names remains unresolved, now explicitly AETS-015's own deferred item rather than this document's.
