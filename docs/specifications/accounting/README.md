...
| [AETS-004 — Journal & Posting Model](AETS-004-Journal-Posting-Model.md) | Active | 1.1.0 | Normative Journal/Journal Line model, debit/credit semantics, posting lifecycle, atomicity, idempotency, append-only rules, reversal/replacement, and 23 `JRN-NNN` invariants |
...
| [AETS-007 — Accounting Commands & Posting Pipeline](AETS-007-Posting-Command.md) | Active | 1.1.0 | The concrete Posting Command contract (fields, identity/idempotency, Source Fingerprint, Actor/Source/Evidence relationship, Journal payload) — the first fully-specified Accounting Command under this number — the Posting Validation Pipeline reconciled to and extending AETS-004 §11/AETS-005 §19, atomic transaction/failure/success semantics, the AI-authority boundary, and 27 `POST-NNN` invariants |
...
| [ATS-007 — Accounting Commands & Posting Pipeline Test Specification](tests/ATS-007-Posting-Pipeline-Test-Specification.md) | Active | 1.1.0 | [AETS-007](AETS-007-Posting-Command.md) | 131 test IDs (`POST-T001`–`POST-T131`) tracing every `POST-NNN` invariant to at least one test; Posting Command construction, tenant/Idempotency-Key/Source-Fingerprint/Actor-Source/Evidence/Account/Money-currency-balance validation tests, duplicate-prevention and atomic-persistence tests (real-PostgreSQL fault injection at every required-write point), failure/success semantics, AI-authority-boundary parity tests, Audit Event and Outbox atomicity, no-network-in-transaction architecture proof, real-PostgreSQL concurrency tests, property-based generators, and 13 golden Posting Command scenarios — `POST-T093`–`POST-T095` (Audit Event) are now implemented against [AETS-010](AETS-010-Audit-Trail-Evidence-Linkage.md)/[ATS-010](tests/ATS-010-Audit-Trail-Test-Specification.md) (M6); `POST-T096`–`POST-T098` (Outbox) remain deferred, unchanged, since AETS-010 deliberately excludes the Outbox Event schema (§2.2) |
