...
| [ATS-007 — Accounting Commands & Posting Pipeline Test Specification](tests/ATS-007-Posting-Pipeline-Test-Specification.md) | Active | 1.0.0 | [AETS-007](AETS-007-Posting-Command.md) | 131 test IDs (`POST-T001`–`POST-T131`) tracing every `POST-NNN` invariant to at least one test; Posting Command construction, tenant/Idempotency-Key/Source-Fingerprint/Actor-Source/Evidence/Account/Money-currency-balance validation tests, duplicate-prevention and atomic-persistence tests (real-PostgreSQL fault injection at every required-write point), failure/success semantics, AI-authority-boundary parity tests, Audit Event and Outbox atomicity, no-network-in-transaction architecture proof, real-PostgreSQL concurrency tests, property-based generators, and 13 golden Posting Command scenarios |
