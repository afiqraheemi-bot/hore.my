...
| [ATS-004 — Journal & Posting Test Specification](tests/ATS-004-Journal-Posting-Test-Specification.md) | Active | 1.4.0 | [AETS-004](AETS-004-Journal-Posting-Model.md) | 161 test IDs (`JRN-T001`–`JRN-T161`) tracing every `JRN-NNN` invariant to at least one test; balance/reversal property-based generators, golden journal cases, atomicity/idempotency/concurrency and tenant-isolation integration tests, Journal reconstitution (Draft/Posted restoration without a posting side effect), real-PostgreSQL Journal persistence adapter tests (header/line mapping, exact `BIGINT` round-trip, malformed-value rejection), and real-PostgreSQL production `journals`/`journal_lines` schema constraint tests (uniqueness, composite-FK same-Tenant Journal/Account integrity, canonical-value `CHECK` constraints, non-negative Money magnitude, migration reversibility) |
