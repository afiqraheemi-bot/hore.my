...
| [ATS-004 — Journal & Posting Test Specification](tests/ATS-004-Journal-Posting-Test-Specification.md) | Active | 1.2.0 | [AETS-004](AETS-004-Journal-Posting-Model.md) | 133 test IDs (`JRN-T001`–`JRN-T133`) tracing every `JRN-NNN` invariant to at least one test; balance/reversal property-based generators, golden journal cases, atomicity/idempotency/concurrency and tenant-isolation integration tests, Journal reconstitution (Draft/Posted restoration without a posting side effect), and real-PostgreSQL Journal persistence adapter tests (header/line mapping, exact `BIGINT` round-trip, malformed-value rejection) |
